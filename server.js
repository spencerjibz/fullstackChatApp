...
let { Duplex, Readable } = require("stream");
...
server.listen(port, () => {
  ...
  if (process.cwd().includes("client")) {
    exec("yarn start", (data, err) =>
      err ? log(err) : log("starting react-client"),
    );
  }
  process.chdir("./client");
  //
  exec("yarn start", (err, done) =>
    err ? log(err) : log("starting react-client"),
  );
});
...
let stream = new Readable({objectMode: true,
  ...
  socket.on("File", (file) => {
    ...
    if (addedUser) {
      ...
      setTimeout(
        () =>
          socket.compress(true).broadcast.emit("sent-file", {
            username,
            sentFile,
          }),
        5,
