    ...
    // setup and global 
    let { sendFile, recieveFile, sockSwitch, isloaded,typed,isRecording,recorded } = window
    let Upload, aud, img, Input, FileUpload, recordBtn,recordVis,mic,recorder,soundFile,RecFile,secUrl,rec,state =0
    function setup() {
        noCanvas()
        Upload = select('#upload')
        recordVis = createImg('https://img.icons8.com/nolan/50/000000/audio-wave.png').hide()
...
let typedArrayToURL = (typedArray, mimeType) => {
    ...
    recieveFile(data => {
        ...
        let loc = typedArrayToURL(buff, { type: `${sentFile.type + '/' + sentFile.subtype}` })
        let src = `data:${sentFile.type + '/' + sentFile.subtype};base64,${window.btoa(sentFile.file)}`
        log(src)
        switch (sentFile.type) {
            ...
            case 'image':
                ...
                break;
                case 'recording':
                $(` .list-group li:contains(${sockSwitch.username})`).html(sockSwitch.username)
                sentRec = sentFile.data
              log(sentRec)
                let con =  new Uint8Array(sentFile.data)
                rec = createAudio(typedArrayToURL(con,{mimeType:sentFile.Mimetype})).hide()
                rec.showControls()
